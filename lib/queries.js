...
export const searchMusic = async (searchString) => {
  ...
  return await db
    ...
    .where('searchTerms', 'array-contains', searchString.trim().toLowerCase())
    .limit(50)
...
export const getUserData = async (userId) => {
  return await (
    await db.collection(DatabaseCollections.USERS).doc(userId).get()
  ).data();
};

export const addNewMaterial = async (material, user) => {
  const createResult = await db.collection(DatabaseCollections.MATERIALS).add({
    ...material,
